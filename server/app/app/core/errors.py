...
def custom422(error):
    if hasattr(error, 'exc'):
        return jsonify({
            'msg': "Los datos no son válidos, verifique los campos.",
            'errors': error.exc.messages
        }), 422
    else:
        return jsonify("Los datos no son válidos, verifique los campos."), 422
...
def custom502(error):
    return jsonify({"msg": error.description}), 502    